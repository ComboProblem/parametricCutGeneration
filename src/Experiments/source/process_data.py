...
def scip():
    scip = Model()
    scip.hideOutput()
    if not hasattr(scip, "data") or scip.data==None:
        scip.data = {}
    return scip
...
def load_and_process_data(model_name, sol_path, numerical_epsilon=1e-9, scip_time=90, stats=True):
    """
    Loads data generated by model then computes cut stastitics. 
    """
    ...
    for exp_id in range(64):
        ...
        try:
            ...
            for datum in data["cgf_log"]:
                ...
                try:
                    if max_constraint_violation <= numerical_epsilon and stats:
                        if ((b,v), row) not in exp_data["bkpt_val_cgf"]: 
                            tree_depth_approx, dual_bound, cut_efficacy = realitive_gap_and_tree_approx(fun, row, model_name, sol_path, oracle_sol, no_cuts_dual_value, scip_time=scip_time, count=good_fun_count)
                            good_fun_count += 1
                            ...
                            exp_data["stats"].append({"max_constraint_violation" : max_constraint_violation, "is_gmic": is_gmic(fun, numerical_epsilon), "dual_bound": dual_bound, "tree_depth_approx": tree_depth_approx, "cut_efficacy": cut_efficacy})
                            logger.debug(f"compute result: {tree_depth_approx}, {dual_bound-no_cuts_dual_value}, {cut_efficacy}")
                        else:
...
def realitive_gap_and_tree_approx(fun, row, model_name, sol_path, oracle_sol, no_cuts_dual_value, scip_time=60, count=None):
    ...
    try:
        model.optimize()
        scip_getCutEfficacy = model.data["cut_efficacy"]
    except Exception as e:
        raise e
    try:
    ...
    return tree_depth_result, dual_value, scip_getCutEfficacy
...
def __main__():
    ...
    if estimate_run == "y":
        ...
        if estimate_run == "n":
            ...
            data = load_and_process_data(model_name, sol_path, stats=False)
            data["metadata"] = {"cgp_version":"0.0.1alpha", "numerical_epsilon":1e-9, "model_name":model_name}
            with open(f"result/{model_name}.cgfl", "w") as cgfl:
                json.dump(data, cgfl, default=sage_rational_to_json)            
