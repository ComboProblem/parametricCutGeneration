...
cut_selection_problem_logger = logging.getLogger(__name__)
cut_selection_problem_logger.setLevel(logging.DEBUG)
...
def log_problem_result(bkpt, val, binvarow, binvc, f):
    cut_selection_problem_logger.info(f"Cut generation problem solved.")
    cut_selection_problem_logger.info(f"breakpoitns: {bkpt}")
    cut_selection_problem_logger.info(f"values: {val}")
    cut_selection_problem_logger.info(f"row: {binvarow}")
    cut_selection_problem_logger.info(f"objective:{binvc}")
    cut_selection_problem_logger.info(f"f: {f}")
...
class cgpTimer:
    ...
    def solver_time_out(self):
        if time.process_time() - self._start_time >= self._max_time:
            cut_selection_problem_logger.warning(f"Solver timed out.")
            return True
        return False
...
class cutGenerationProblem:
    ...
    def _algorithm_full_space(self, binvarow, binvc, f):
        ...
        problem_timer = cgpTimer(Self._max_cgf_solver_time)
        ...
        if self._prove_seperator:
            ...
            if not res:
                cut_selection_problem_logger.error(f"minimality of  {pi_p}: {res}")
        return pi_p
    ...
    def _algorithm_bkpt_as_param(self, binvarow, binvc, f):
        ...
        if num_bkpt == 2:
            cut_selection_problem_logger.debug("Parsed row data suggests to use GMIC.")
            cut_selection_problem_logger.debug("Dim of value polyhedron: 0")
            pi_p =  gmic(frac_f)
            log_problem_result(sparse_bkpt, [0, 1], binvarow, binvc, f)
            if self._prove_seperator:
                # we always have a separator here.
                cut_selection_problem_logger.debug(f"The minimality of the found cgf is {True}")
        ...
        cut_selection_problem_logger.debug(f"Dim of value polyhedron :{value_polyhedron.upstairs().ambient_dim()}")
        ...
        if self._prove_seperator:
            ...
            cut_selection_problem_logger.info(f"Minimality of cgf: {res}")
        return pi_p
